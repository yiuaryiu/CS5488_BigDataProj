...
object DbscanEuclidean {
  ...
  def main(args: Array[String]) {
    ...
    def dbscan(points: Array[(Double, Double)], eps: Double, minPoints: Int): Array[(Double, Double)] = {
      println("hereeee dbscan start")
      val visited = Array.fill(points.length)(false)
      val clusterCenters = ListBuffer[(Double, Double)]()

      def expandCluster(point: (Double, Double), neighbors: Array[(Double, Double)]): Unit = {
        visited(points.indexOf(point)) = true
        val clusterPoints = ListBuffer(point) // Initialize cluster points with the current point

        var i = 0
        while (i < neighbors.length) {
          val neighbor = neighbors(i)
          val neighborIndex = points.indexOf(neighbor)
          if (!visited(neighborIndex)) {
            visited(neighborIndex) = true
            val neighborNeighbors = getNeighbors(neighbor, points, eps)
      ...
      println("hereeee dbscan finished")
